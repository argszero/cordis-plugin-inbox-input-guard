...
MUTATIONS = [
    ...
    ('the guard no longer records what it did',
     ...
     '    const record = (violation) => {\n        violations.push(violation);'),
    ('the pending sweep is never run: a mid-turn injection stays in the projection',
     '        sanitizePending(payload.agent);',
     '        void 0;'),
    ('the pending sweep walks forward: removing one entry shifts the next',
     '            for (let index = list.length - 1; index >= 0; index -= 1) {',
     '            for (let index = 0; index < list.length; index += 1) {'),
    ('the pending repair puts the offending value back instead of a message',
     '                    agent.inbox.splice(target, index, 1, [repairOf(text, violation.valueKind)]);',
     '                    agent.inbox.splice(target, index, 1, [value]);'),
    ('pending violations are mislabelled as claimed ones',
     "                    origin: 'pending',",
     "                    origin: 'claimed',"),
    ('apply loses its config default: the bare-object mount throws instead of working',
